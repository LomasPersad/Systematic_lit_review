...
modified from main.py in /Gscholar
...
def get_citecount(cite_tag):
    ...
    for i in cite_tag:
        ...
        cite = i.find('div', 'gs_ri').find(
            'div', 'gs_fl').find_all('a')[2].string
        # print(cite)
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    abs_txt = paper_doc.find("div", {"class": class_name}).find(
                        'p').text.replace('\xa0', '')
...
def get_author_year_publi_info(authors_tag):
    ...
    for i in range(len(authors_tag)):
        ...
        try:
            ...
            publication.append('na')
        try:
            author = authortag_text[0] + ' ' + \
                re.sub(',', '', authortag_text[1])
...
for i in range(0, 1510, 10):  # start, final number of papers-n, n=10 (since 10 results perpage)

    # get url for the each page
    url = "https://scholar.google.com/scholar?start={}&as_sdt=0%2C24&inst=12058184521150304743&q=specific+tension+of+human+muscle&btnG=".format(
        i)
    # print(url)
    ...
    final = add_in_paper_repo(
        papername, year, author, cite, publication, link, abstract_txt, key_sentence)
