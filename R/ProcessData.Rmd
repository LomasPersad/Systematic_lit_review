...
# trainingtype muscleName jointAction  aveForce anatomicalLocation  FastSlow
# fl_method ma_method pa_method mv_method f_method t_method

plotFacet(data_r,"jointAction","placeholder")
...
data_r_filtered <- data_r %>% filter(newScore > (24))
...
ggplot(highest_score_per_muscle2, aes(x = reorder(muscleName, -aveST), y = aveST, color = newScore)) +
  geom_point() +  # Set custom color palette
...
df$Normalized_Score <- (df$newScore - min(df$newScore)) / (34 - min(df$newScore))
...
df$studyID <- paste(data_r$Study.ID,'(',data_r$Muscle,',',round(df$Adjusted_Score,1), 'N/cm² )')
...
# Plotting the average values
 ...
 fig9bcomplete <- ggplot(df, aes(y = Adjusted_Score)) +
  geom_boxplot(outlier.shape = NA, width = 0.08) +  # Turn off the outliers outlier.shape = NA
  geom_jitter(aes(x = 0, color=studyID), width = 0.009, alpha = 0.5) +  # Show all data points with jitter
   # stat_summary(aes(x = 0),fun = median, geom = "point", shape = 18, size = 0, color = "red", position = position_dodge(0.75))+
