...
# 30 papers
# paperstats_r %>%
#   distinct(CovidenceID) %>%
#   count()


matrixStats::weightedMedian(data_r$aveST,data_r$newScore/31)
...
ggplot(data_r, aes(y = aveST)) +
  geom_boxplot(outlier.shape = NA, width = 0.08) +  # Turn off the outliers
  geom_jitter(aes(x = 0, color = Muscle, shape = anatomicalLocation), width = 0.009, alpha = 1) +  # Jittered points
  stat_summary(aes(x = 0), fun = function(y) weightedMedian(y, data_r$newScore/31), geom = "point", shape = 18, size = 3, color = "red") +  # Weighted median
  annotate("text", x = 0, y = weightedMedian(data_r$aveST, data_r$newScore/31), 
           label = round(weightedMedian(data_r$aveST, data_r$newScore/31), 1), 
           vjust = -0.5, hjust = -0.4, size = 4, color = "red") +
  theme_pubr(legend = 'right') +
  labs(y = expression("Specific tension "~(N/cm^2)), color = "Muscle", shape = "Anatomical location") +
...
# Plotting with customized linear regression per facet -old 
...
# Convert 5c to box plot
# Define bins
breaks <- seq(0, 100, by = 10)
labels <- paste0(seq(0, 90, by = 10), "-", seq(10, 100, by = 10))
melted_df$bin <- cut(melted_df$percentage, breaks = breaks, labels = labels, right = FALSE)

box5c <-ggboxplot(melted_df, x = "bin", y = "aveST",
          palette = "jco", add = "jitter") +
   # stat_compare_means(method = "t.test", 
   #                   comparisons = list(c("40-50", "50-60")), 
   #                   hide.ns = TRUE, 
  ...
  theme(strip.background = element_blank(),  # Remove facet strip background for cleaner appearance
        ...
        axis.text = element_text(size = 12),
        axis.text.x = element_text(size = 9))  # Adjust facet strip text appearance
