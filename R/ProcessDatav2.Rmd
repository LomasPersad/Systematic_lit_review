...
#   geom_point(aes(color = Muscle, shape=anatomicalLocation)) +  #musclename 
#   labs(
#     y = "Author",
...
data_r <- data_r %>%
 ...
 ## Plot with BOLD legend text
 # fig8complete <- ggplot(data_r, aes(y = aveST)) +
 #  geom_boxplot(outlier.shape = NA, width = 0.08) +
 #  geom_jitter(aes(x = 0, color = Muscle, shape = anatomicalLocation), width = 0.009, alpha = 1) +
 #  annotate("text", x = 0, y = median(data_r$aveST), label = median(data_r$aveST), vjust = -0.5, hjust = -0.4, size = 4) +
 ...
 #  labs(color = "Muscle", shape = "Anatomical location") +
 #  scale_color_manual(values = custom_colors2)
...
# columns = c("%MHC-1", "%MHC-2A", "%MHC-2X")
# tbl <- json_data %>%
#   gt() %>%
#   tab_spanner(label = "Fiber composition (%)", columns = c("Type 1", "Type 2A", "Type 2X")) %>%
#   tab_style(
#     style = list(
#       cell_text(color = "black", align = "center")
#     ),
#     locations = list(cells_column_labels(), cells_stubhead(),cells_body()) 
#   )
# tbl %>%gtsave("Fig5_table.png")


# Assuming tbl is your table object
# n_rows <- nrow(json_data)
# half_rows <- ceiling(n_rows / 2)
# half_rows
# 
# tbl_left <- json_data[1:half_rows, ]
# tbl_right <- json_data[(half_rows + 1):n_rows, ]
# 
# 
# tbla <- tbl_left %>%
#   gt() %>%
...
fig7complete
saveFig(fig7complete,'Fig5 ST_musclegrps',12,14)
