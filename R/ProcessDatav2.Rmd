...
Fiber_ST <- ggscatter(
  ...
  stat_cor(label.y = 70) +
  stat_regline_equation(label.y = 67)+
  labs(x = "Fiber composotion (%)", y = expression("Specific tension "~(N/cm^2)),
...
tbl <- json_data %>%
  gt() %>%
  tab_spanner(label = "Fiber composition", columns = c("%MHC-1", "%MHC-2A", "%MHC-2X")) %>%
  tab_style(
    style = list(
...
saveFig(fig7complete,'breakdown4b',12,14)
