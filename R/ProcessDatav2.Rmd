...
ggscatter(data_r, x = "Type1", y = "aveST",color = "muscleName",
                           legend.title = "Anatomical location",  # Legend title
                           ellipse = FALSE, mean.point = FALSE,star.plot = FALSE
) 

ggscatter(data_r, x = "Type2A", y = "aveST",color = "muscleName",
                           legend.title = "Anatomical location",  # Legend title
                           ellipse = FALSE, mean.point = FALSE,star.plot = FALSE
) 


ggscatter(data_r, x = "Type2X", y = "aveST",color = "muscleName",
                           legend.title = "Anatomical location",  # Legend title
                           ellipse = FALSE, mean.point = FALSE,star.plot = FALSE
) 
...
# 4. Violin Plot
ggplot(data_r, aes(x = Type1, y = aveST, fill = muscleName)) +
  geom_violin() +
  labs(title = "Violin Plot", x = "Fiber Type", y = "Specific Tension")

# 5. Line Plot
ggplot(data_r, aes(x = Type1, y = aveST, group = Type1, color = muscleName)) +
  geom_line() +
  labs(title = "Line Plot", x = "Muscle ID", y = "Specific Tension")

# 6. Correlation Plot
correlation_matrix <- cor(select(Type1, -Muscle_ID))  # Calculate correlation matrix
correlation_plot <- ggplot(data = NULL, aes(x = colnames(correlation_matrix), y = colnames(correlation_matrix))) +
  geom_tile(aes(fill = correlation_matrix), color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  theme_minimal() +
  labs(title = "Correlation Plot", x = "Variable", y = "Variable", fill = "Correlation")


  # facet_grid(anatomicalLocation ~ ., scales = "fixed", space = "free") +
...
data_r$anatomicalLocation <- factor(data_r$anatomicalLocation, levels = c("Upper arm", "Upper leg", "Lower leg"))

data_r$muscleName <- factor(data_r$muscleName, levels = muscle_order)



#Plot PCSA and FL vs ST 

Plot_PCSA <- function(grpVariable){
  # Create a scatter plot with a beautiful template
...
# Create a scatter plot with a beautiful template
FLvST1 <- ggscatter(data_r, x = {{grpVariable}}, y = "aveST",color = "muscleName",
                           legend.title = "Anatomical location",  # Legend title
                           ellipse = FALSE, mean.point = FALSE,star.plot = FALSE
) +
  facet_grid(anatomicalLocation ~ ., scales = "fixed", space = "free") +
...
FLvST1 <- FLvST1+
  theme_pubr() + labs(y = expression("Specific tension "~(N/cm^2)), x=expression("PCSA "~(cm^2))) + theme(legend.position = "right")           # Apply the ggpubr theme
...
Plot_PCSA <- function(grpVariable) { 
  # Use a predefined color palette with more variations
  ...
  # Create a scatter plot with a beautiful template
  FLvST1 <- ggscatter(data_r, x = {{grpVariable}}, y = "aveST", color = "muscleName", 
                      legend.title = "Muscle",  # Legend title
                      ellipse = FALSE, mean.point = FALSE, star.plot = FALSE
  ) + scale_color_manual(values = my_Color_palette) +
    facet_wrap(anatomicalLocation ~ ., scales = "fixed") +
    xlim(0, max(data_r$aveFL))
  
  FLvST1 <- FLvST1  +
    theme_pubr() + 
    labs(y = expression("Specific tension "~(N/cm^2)), x = expression("PCSA "~(cm^2))) +
    theme(legend.position = "right")  # Apply the ggpubr theme
...
Plot_PCSA <- function(grpVariable) { 
  # Use a predefined color palette with more variations
  # my_Color_palette <- paletteer_c("grDevices::Dark 3", 8)
  my_Color_palette <- brewer.pal(8, "Set2")
  # my_Color_palette <-custom_colors
  
  # Group the data by anatomicalLocation
  grouped_data <- data_r %>% group_by(anatomicalLocation)
  
  # Create an empty list to store individual plots
  plots_list <- list()
  
  Location_order <- c("Upper arm", "Upper leg", "Lower leg") #unique(data_r$anatomicalLocation)
  # Iterate over each group and create a scatter plot
  for (group_name in Location_order) {
    # Filter data for the current group
    group_data <- grouped_data %>% filter(anatomicalLocation == group_name)
    
    # Create scatter plot for the current group
    plot <- ggscatter(group_data, x = {{grpVariable}}, y = "aveST", color = "muscleName",
                      legend.title = paste(group_name," muscles"),  # Legend title
                      ellipse = FALSE, mean.point = FALSE, star.plot = FALSE
    ) + scale_color_manual(values = my_Color_palette)+
  ...
  # Combine all individual plots into a single plot arranged in rows
  combined_plot <- plot_grid(plotlist = plots_list, nrow = length(unique(data_r$anatomicalLocation)))
...
chosen <- 'avePCSA'
STpcsa <- Plot_PCSA(chosen)
STpcsa
saveFig(STpcsa,'STvsPCSA',8,6)


Plot_FL <- function(grpVariable) { 
  # Use a predefined color palette with more variations
  # my_Color_palette <- paletteer_c("grDevices::Dark 3", 8)
  my_Color_palette <- brewer.pal(8, "Set2")
  # my_Color_palette <-custom_colors
  ...
  for (group_name in Location_order) {
    ...
    xlim(0, 20) + ylim(0, 80) +
      labs(y = expression("Specific tension "~(N/cm^2)), x = expression("Fiber length "~(cm))) +
      theme(legend.position = "right")  # Apply the ggpubr theme
...
chosen <- 'aveFL'
ST_FL <- Plot_FL(chosen)
ST_FL

saveFig(ST_FL,'STvsFL',8,6)


# my_Color_palette <- paletteer_c("grDevices::Zissou 1", 8)
# ggplot()+ geom_point(data = data_r, aes(x = avePCSA, y = aveST, color = muscleName, shape = anatomicalLocation),size=2)
